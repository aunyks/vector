function render() {
	var grey = '#bdc3c7';// BACKGROUND
	if (gameState === 0) {
		grey = "rgb(220, 223, 225)";// BACKGROUND
	}
...
function renderBeginningText() {
	...
	renderText((trueCanvas.width)/2 + 2 * settings.scale,upperheight-0*settings.scale, fontSize, '#0E8C3A', input_text);// TEXT COLOR
	renderText((trueCanvas.width)/2 + 2 * settings.scale,upperheight+33*settings.scale, fontSize, '#0E8C3A', action_text);// TEXT COLOR
    if (!mob) {
	    drawKey("",(trueCanvas.width)/2 + 2 * settings.scale-2.5,upperheight+38*settings.scale);
    }

	renderText((trueCanvas.width)/2 + 2 * settings.scale,lowerheight,fontSize, '#0E8C3A', score_text);// TEXT COLOR
}
